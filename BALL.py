...
# --- empirical.rdoc_score_llm (empirical/rdoc_score_llm.py) ---
SRC_EMPIRICAL_RDOC_SCORE_LLM = r'''
"""Faithful McCoy and Perlis large language model RDoC scorer.

Reimplements the scorer from McCoy TH, Perlis RH. Characterizing research
domain criteria symptoms among psychiatric inpatients using large language
models. J Mood Anxiety Disord. 2024;8:100079. Each clinical note is scored
independently by a large language model with the paper's exact prompt, on a
0 to 10 scale for each of the six NIMH Research Domain Criteria domains, and
the model returns a JSON object. The model is called with temperature 0 and is
re-initialized for every note so there is no carryover between notes, matching
the paper.

The default model is gpt-4-1106-preview, the model the paper used. Override with
--model for a current model if that snapshot is retired.

API key. The OpenAI key is read from the OPENAI_API_KEY environment variable, or
from a local key file passed with --key-file (default empirical/.openai_key if
present). The key is never printed or written to any output.

Caching. Each unique note is scored once. Results are cached in
empirical/derived/rdoc_llm_cache.jsonl keyed by a hash of the model and note
text, so runs are resumable and identical notes (common for short diagnosis
stubs) are not re-billed. The cache stores hashes and scores, not note text.

Privacy. Notes are sent to the configured model endpoint. Use only a
HIPAA-compliant deployment, as the paper did. The dry run sends nothing.
"""
...
OUT = ROOT / "derived"
PHI_OUT = ROOT / "derived_phi"
CACHE = OUT / "rdoc_llm_cache.jsonl"

# Verbatim prompt from McCoy and Perlis 2024 (J Mood Anxiety Disord 8:100079).
PROMPT = (
    "You are a skilled psychiatrist scoring an emergency room clinical note in "
    "terms of how the patient symptoms over the past 24 h reflect the 6 NIMH "
    "Research Domain Criteria (RDoC): Negative Valence Systems, Positive Valence "
    "Systems, Cognitive Systems, Social Processes, Arousal and Regulatory "
    "Systems, and Sensorimotor Systems. Remember that substance use can be "
    "reflected as Positive Valence symptom. Notes are scored on a 0-10 scale to "
    "capture the magnitude of documented symptoms relevant in a given domain. "
    "Score 0 if no symptoms are present and functioning is normal, 1-3 mild "
    "symptoms, 4-6 moderate, 7-9 severe, 10 extremely severe. Generally moderate "
    "requires treatment and severe requires hospitalization. Return only a JSON "
    "object with each score, Negative, Positive, Cognitive, Social, Arousal, and "
    "Sensorimotor."
)
PROMPT_VERSION = "mccoy_perlis_2024_v1"

DOMAIN_KEYS = ["Negative", "Positive", "Cognitive", "Social", "Arousal", "Sensorimotor"]
DOMAIN_COLS = {
    "Negative": "rdoc_negative_valence",
    "Positive": "rdoc_positive_valence",
    "Cognitive": "rdoc_cognitive_systems",
    "Social": "rdoc_social_processes",
    "Arousal": "rdoc_arousal_regulatory",
    "Sensorimotor": "rdoc_sensorimotor",
}


def _resolve_key(key_file: str | None) -> str | None:
    if os.environ.get("OPENAI_API_KEY"):
        return os.environ["OPENAI_API_KEY"]
    candidates = [key_file] if key_file else []
    candidates.append(str(ROOT / ".openai_key"))
    for c in candidates:
...
def _note_hash(model: str, text: str) -> str:
    return hashlib.sha256(f"{model}|{PROMPT_VERSION}|{text}".encode("utf-8")).hexdigest()
...
def _load_cache() -> dict:
    ...
    if CACHE.exists():
        for line in CACHE.read_text(encoding="utf-8").splitlines():
            ...
            try:
                rec = json.loads(line)
                cache[rec["hash"]] = rec["scores"]
            except Exception:
                continue
    return cache


def _append_cache(h: str, scores: dict) -> None:
    with CACHE.open("a", encoding="utf-8") as fh:
        fh.write(json.dumps({"hash": h, "scores": scores}) + "\n")


def _parse_scores(content: str) -> dict | None:
    """Parse the model JSON into the six domain scores, clipped to 0-10."""
...
def _score_one(client, model: str, text: str, retries: int = 4) -> dict | None:
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                print(f"  [warn] giving up on a note after {retries} attempts: {exc}", file=sys.stderr)
                return None
...
def main() -> None:
    p = argparse.ArgumentParser(description="McCoy/Perlis LLM RDoC scorer.")
    p.add_argument("--input", default=str(PHI_OUT / "rdoc_input.csv"))
    p.add_argument("--model", default="gpt-4-1106-preview")
    p.add_argument("--tiers", default="rich_narrative,diagnosis_only",
                   help="Comma-separated note_tier values to score. Use 'all' for every note.")
    p.add_argument("--min-chars", type=int, default=1, help="Skip notes shorter than this.")
    p.add_argument("--limit", type=int, default=None, help="Score at most this many unique notes (for piloting).")
    p.add_argument("--key-file", default=None)
    p.add_argument("--dry-run", action="store_true", help="Estimate scope and cost, send nothing.")
    args = p.parse_args()

    df = pd.read_csv(args.input)
    if args.tiers.strip().lower() != "all" and "note_tier" in df.columns:
        keep = set(t.strip() for t in args.tiers.split(","))
        df = df[df["note_tier"].isin(keep)]
    df = df[df["note_char_count"].fillna(0) >= args.min_chars].copy()
    df["combined_note_text"] = df["combined_note_text"].astype(str)

    # Deduplicate identical note texts so each unique note is scored once.
    unique_texts = df["combined_note_text"].drop_duplicates().tolist()
    cache = _load_cache()
    to_score = [t for t in unique_texts if _note_hash(args.model, t) not in cache]

    approx_tokens = sum((len(PROMPT) + len(t)) // 4 + 30 for t in to_score)
    print(f"notes selected: {len(df)} rows, {len(unique_texts)} unique texts")
    print(f"already cached: {len(unique_texts) - len(to_score)}; to score now: {len(to_score)}")
    print(f"approx input+output tokens for new calls: {approx_tokens:,}")
    if args.limit:
        print(f"limit: will score at most {args.limit} new unique notes this run")

    if args.dry_run:
        print("dry run: no API calls made. Set OPENAI_API_KEY or --key-file and drop --dry-run to score.")
        return
    ...
    if not key:
        ...
        print("  - set OPENAI_API_KEY in the environment, or")
        print(f"  - write the key to {ROOT/'.openai_key'} (one line), or")
        print("  - pass --key-file PATH")
        print("Then re-run without --dry-run. (Use a HIPAA-compliant deployment for PHI.)")
        sys.exit(2)

    try:
        from openai import OpenAI
    except Exception as exc:  # noqa: BLE001
        print(f"openai SDK import failed: {exc}. Run: pip install openai", file=sys.stderr)
        sys.exit(2)
    ...
    for text in to_score:
        if args.limit and scored >= args.limit:
            break
        h = _note_hash(args.model, text)
        scores = _score_one(client, args.model, text)
        if scores is None:
            continue
        _append_cache(h, scores)
        cache[h] = scores
        scored += 1
        if scored % 50 == 0:
            print(f"  scored {scored} new notes...", flush=True)
    print(f"scored {scored} new unique notes this run")

    # Assemble per-note score table from the cache.
    rows = []
    for _, r in df.iterrows():
        h = _note_hash(args.model, r["combined_note_text"])
        s = cache.get(h)
        if s is None:
            continue
        row = {"id": r["id"], "AppointmentFID": r.get("AppointmentFID"),
               "ServiceDate": r.get("ServiceDate"), "day": r.get("day"),
               "note_tier": r.get("note_tier"), "note_char_count": r.get("note_char_count")}
        for k in DOMAIN_KEYS:
            row[DOMAIN_COLS[k]] = s[k]
        rows.append(row)
    out = pd.DataFrame(rows)
    out.to_csv(OUT / "rdoc_scores_llm.csv", index=False)

    manifest = {
        "scorer": "McCoy/Perlis 2024 LLM RDoC scorer (faithful reimplementation)",
        "reference": "McCoy TH, Perlis RH. J Mood Anxiety Disord. 2024;8:100079.",
        "model": args.model,
        "temperature": 0,
        "prompt_version": PROMPT_VERSION,
        "scale": "0-10 per domain",
        "domains": list(DOMAIN_COLS.values()),
        "notes_scored": int(len(out)),
        "unique_texts_scored": int(len(cache)),
        "tiers": args.tiers,
        "privacy": "notes sent to the configured model endpoint; use a HIPAA-compliant deployment",
    }
    (OUT / "rdoc_scores_llm_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    print(f"wrote {OUT/'rdoc_scores_llm.csv'} ({len(out)} rows)")
...
MODULE_SOURCES = {
    ...
    'empirical.rdoc_score_llm': ('empirical/rdoc_score_llm.py', SRC_EMPIRICAL_RDOC_SCORE_LLM),
}
...
COMMANDS = {
    ...
    'empirical-score-rdoc-llm': 'empirical.rdoc_score_llm',
}
